...
const subscriptionQueries = {
  ...
  getById(id) {
    const db2 = getDatabase();
    return db2.prepare("SELECT * FROM subscriptions WHERE id = ?").get(id);
  },
  create(data) {
    const db2 = getDatabase();
    const result = db2.prepare(`
  ...
  update(id, data) {
    ...
    fields.push("updated_at = CURRENT_TIMESTAMP");
    fields.push("sync_status = 'pending'");
    db2.prepare(`UPDATE subscriptions SET ${fields.join(", ")} WHERE id = @id`).run(values);
    return this.getById(id);
  },
  delete(id) {
    const db2 = getDatabase();
    ...
    const result = db2.prepare("DELETE FROM subscriptions WHERE id = ?").run(id);
    return result.changes > 0;
  },
  ...
  getActiveClientSubscription(clientId) {
    const db2 = getDatabase();
    return db2.prepare(`
      SELECT cs.*, s.name as subscription_name, s.price as subscription_price
      FROM client_subscriptions cs
  ...
  markAsPaid(clientSubscriptionId, paymentDate) {
    const db2 = getDatabase();
    const date = paymentDate || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    db2.prepare(`
    ...
    return db2.prepare(`
      ...
      JOIN subscriptions s ON s.id = cs.subscription_id
      WHERE cs.id = ?
    `).get(clientSubscriptionId);
  },
  incrementVisit(clientSubscriptionId) {
    const db2 = getDatabase();
    const result = db2.prepare(`
      UPDATE client_subscriptions 
  ...
  removeClientSubscription(clientSubscriptionId) {
    const db2 = getDatabase();
    const result = db2.prepare("DELETE FROM client_subscriptions WHERE id = ?").run(clientSubscriptionId);
    return result.changes > 0;
  },
...
function setupIpcHandlers() {
  ...
  ipcMain.handle("db:subscriptions:getClientSubscriptions", (_, clientId) => subscriptionQueries.getClientSubscriptions(clientId));
  ipcMain.handle("db:subscriptions:getActiveClientSubscription", (_, clientId) => subscriptionQueries.getActiveClientSubscription(clientId));
  ...
  ipcMain.handle("db:subscriptions:removeClientSubscription", (_, id) => subscriptionQueries.removeClientSubscription(id));
